...
class AzureDevOpsIntegrations {
    ...
    private static _instance : AzureDevOpsIntegrations = new AzureDevOpsIntegrations();
    ...
    public async getInitiative(idInitiative:string) : Promise<Initiative | undefined>{
        if(configInstance()?.onValidate()){
            const initiativeController : InitiativeController = new InitiativeController();
...
const azureDevOpsIntegrationInstance : AzureDevOpsIntegrations = AzureDevOpsIntegrations.getInstance();
