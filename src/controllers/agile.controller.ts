import { AzureGetProjectsItemResponse, AzureGetWorkItemResponse, AzureWorkItem } from "../models";
...
export class AgileController  {
     ...
     */
    async getByInitiative(id: string): Promise<Array<AzureWorkItem> | undefined> {
    ...
    async getChilds(ids:string[]){
        ...
        const azureGetWorkItemResponse : AzureGetWorkItemResponse[] | undefined = await this.azureClient.getWorkItems(ids);
           if(azureGetWorkItemResponse){
                azureGetWorkItemResponse.forEach((workItem : AzureGetWorkItemResponse) => {
                    const azureWorkitem : AzureWorkItem = new AzureWorkItem(workItem);
     ...
     */
    async getProjects(): Promise<Array<AzureGetProjectsItemResponse> | undefined> {
