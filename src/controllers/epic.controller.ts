...
export class EpicController {
    ...
    get(workItems: AzureWorkItem[] | undefined): Array<Epic> | undefined {
        if (workItems) {
            ...
            const epicsWorkItems = workItems.filter(
                (workItem: AzureWorkItem) =>
                    workItem.type == configInstance()?.getTypes()?.Epic,
            );
     ...
     */
    getAttachedFiles(
        workItems: AzureWorkItem[] | undefined,
    ): { wiki: string; attachedFiles: string[] } | undefined {
        if (workItems) {
