import { WorkItemLinkType, WorkItemStates, WorkItemTypes } from "../../categories";
import {configInstance} from "../../config/configInstance";
import { Person, Project } from "../agile";
import { AzureGetWorkItemRelationResponse, AzureGetWorkItemResponse } from "./getWorkItems";
   ...
   //Fecha en la cuál se crea el elemento de trabajo
    ...
    constructor(workItem: AzureGetWorkItemResponse) {
        ...
        if (workItem.fields) {
            ...
            if (type == configInstance()?.getTypes()?.Epic) this.type = WorkItemTypes.Epic;
            else if (type == configInstance()?.getTypes()?.Feature) this.type = WorkItemTypes.Feature;
            else if (type == configInstance()?.getTypes()?.UserStory) this.type = WorkItemTypes.UserStory;
            else if (type == configInstance()?.getTypes()?.Task) this.type = WorkItemTypes.Task;
            else this.type = WorkItemTypes.Unknow;

            const state = workItem.fields["System.State"];
            if (configInstance()?.getStates()?.Cancelled.includes(state)) this.state = WorkItemStates.Cancelled;
            else if (configInstance()?.getStates()?.Doing.includes(state)) this.state = WorkItemStates.Doing;
            else if (configInstance()?.getStates()?.Done.includes(state)) this.state = WorkItemStates.Done;
            else if (configInstance()?.getStates()?.ToDo.includes(state)) this.state = WorkItemStates.ToDo;
            else this.state = WorkItemStates.Unknow;

            const assignedToItem = workItem.fields["System.AssignedTo"] ?? { displayName: '', uniqueName: '' };
            if (assignedToItem) {
                this.assignedTo = {
            ...
            const createdByItem = workItem.fields["System.CreatedBy"] ?? { displayName: '', uniqueName: '' };
            if (createdByItem) {
                this.createdBy = {
            ...
            const architectItem = workItem.fields["Custom.Arquitecto"] ?? { displayName: '', uniqueName: '' };
            if (architectItem) {
                this.architect = {
            ...
            const scrumMasterItem = workItem.fields["Custom.ScrumMaster"] ?? { displayName: '', uniqueName: '' };
            if (scrumMasterItem) {
                this.scrumMaster = {
            ...
            const productOwnerItem = workItem.fields["Custom.Product_Owner"] ?? { displayName: '', uniqueName: '' };
            if (productOwnerItem) {
                this.productOwner = {
            ...
            const proxyItem = workItem.fields["Custom.GestordeRequerimientos"] ?? { displayName: '', uniqueName: '' };
            if (proxyItem) {
                this.proxy = {
            ...
            const techLeaderItem = workItem.fields["Custom.Desarrollador_Sistema_1"] ?? { displayName: '', uniqueName: '' };
            if (techLeaderItem) {
                this.techLeader = {
            ...
            const affectedSystemsSplit : string[] = workItem.fields['Custom.SistemasAfectados']?.split(";");
            this.affectedSystems = new Array<string>();
            affectedSystemsSplit?.forEach((item) => {this.affectedSystems?.push(item)});

            this.linkWiki = workItem.fields['Custom.Link_Documento_Arquitectura'];
            
            this.teamProject = workItem.fields['System.TeamProject'];
            
            const changedByItem = workItem.fields["System.ChangedBy"] ?? { displayName: '', uniqueName: '' };
            if (changedByItem) {
                this.changedBy = {
        ...
        if(workItem.relations){
            workItem.relations?.forEach((relation:AzureGetWorkItemRelationResponse) => {
                ...
                const idSplit :string[] | undefined = relation.url?.split("/");
                relation.id = idSplit?.reverse()[0];

                if(this.childs){
                    if(relation.rel == WorkItemLinkType.Child) if(relation.id) this.childs.push(relation.id)
                }
                if(this.parents){
                    if(relation.rel == WorkItemLinkType.Parent) if(relation.id) this.parents.push(relation.id)
                }
                if(this.links){
                    if(relation.rel == WorkItemLinkType.Hyperlink || relation.rel == WorkItemLinkType.AttachedFile) if(relation.url) this.links.push(relation.url)
                }
            } )
        }
    }
