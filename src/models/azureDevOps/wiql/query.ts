...
export function query(numIniciativa: string, Epic = configInstance()?.getTypes()?.Epic, Feature = configInstance()?.getTypes()?.Feature, UserStory = configInstance()?.getTypes()?.UserStory, Task = configInstance()?.getTypes()?.Task)
{
