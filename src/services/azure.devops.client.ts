...
export class AzureDevOpsClient {
    private createConnection(): AxiosInstance {
        ...
        instance.defaults.baseURL = baseURI(
            configInstance()?.getOrganization() ?? '',
        );
        instance.defaults.headers.common['Authorization'] =
            'Basic' + btoa('' + ':' + configInstance()?.getToken() ?? '');
    ...
    async getProjects(): Promise<AzureGetProjectsItemResponse[] | undefined> {
        const response =
            await this.createConnection().get<AzureGetProjectsResponse>(
                getProjectsURI(),
            );
    ...
    async getWorkItems(
        ids: string[],
    ): Promise<AzureGetWorkItemResponse[] | undefined> {
        const response = await this.createConnection().post(
            '/_apis/wit/workitemsbatch?api-version=7.1-preview.1',
            { ids: ids, $expand: 'relations' },
        );
        return response.data?.value;
    }
    ...
    async wiql(id: string): Promise<string[] | undefined> {
        const wilqResponse =
            await this.createConnection().post<AzureWiqlResponse>(
                '/_apis/wit/wiql?api-version=7.1-preview.2',
                { query: query(id) },
            );
        const items: string[] = [];
        wilqResponse?.data?.workItems?.forEach(
            (item: AzureWiqlItemResponse) => {
